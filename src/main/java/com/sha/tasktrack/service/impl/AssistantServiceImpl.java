...
public class AssistantServiceImpl implements AssistantService {
    ...
    private String syncResponse(String question) {
        try {
            ...
            String enrichedPrompt = "You are a helpful assistant. If you are unsure, say 'I don't know' instead of guessing. " + "Answer clearly and factually.\n\n" + "Question: " + question;

            // ✅ Build request payload
            JSONObject payload = new JSONObject().put("messages", new JSONArray().put(new JSONObject().put("role", "user").put("content", new JSONArray().put(new JSONObject().put("text", enrichedPrompt))))).put("inferenceConfig", new JSONObject().put("maxTokens", 200).put("temperature", 0.5));

            // ✅ Create Bedrock request
            InvokeModelRequest request = InvokeModelRequest.builder().modelId(MODEL_ID).body(SdkBytes.fromUtf8String(payload.toString())).contentType("application/json").accept("application/json").build();
